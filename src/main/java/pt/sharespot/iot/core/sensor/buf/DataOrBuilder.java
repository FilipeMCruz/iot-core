// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: src/main/resources/sensor.proto

package pt.sharespot.iot.core.sensor.buf;

public interface DataOrBuilder extends
    // @@protoc_insertion_point(interface_extends:pt.sharespot.iot.core.Data)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <code>string data_id = 1;</code>
   * @return The dataId.
   */
  java.lang.String getDataId();
  /**
   * <code>string data_id = 1;</code>
   * @return The bytes for dataId.
   */
  com.google.protobuf.ByteString
      getDataIdBytes();

  /**
   * <code>int64 reported_at = 2;</code>
   * @return The reportedAt.
   */
  long getReportedAt();

  /**
   * <code>.pt.sharespot.iot.core.Device device = 3;</code>
   * @return Whether the device field is set.
   */
  boolean hasDevice();
  /**
   * <code>.pt.sharespot.iot.core.Device device = 3;</code>
   * @return The device.
   */
  pt.sharespot.iot.core.sensor.buf.Device getDevice();
  /**
   * <code>.pt.sharespot.iot.core.Device device = 3;</code>
   */
  pt.sharespot.iot.core.sensor.buf.DeviceOrBuilder getDeviceOrBuilder();

  /**
   * <code>map&lt;int32, .pt.sharespot.iot.core.SensorData&gt; measures = 4;</code>
   */
  int getMeasuresCount();
  /**
   * <code>map&lt;int32, .pt.sharespot.iot.core.SensorData&gt; measures = 4;</code>
   */
  boolean containsMeasures(
      int key);
  /**
   * Use {@link #getMeasuresMap()} instead.
   */
  @java.lang.Deprecated
  java.util.Map<java.lang.Integer, pt.sharespot.iot.core.sensor.buf.SensorData>
  getMeasures();
  /**
   * <code>map&lt;int32, .pt.sharespot.iot.core.SensorData&gt; measures = 4;</code>
   */
  java.util.Map<java.lang.Integer, pt.sharespot.iot.core.sensor.buf.SensorData>
  getMeasuresMap();
  /**
   * <code>map&lt;int32, .pt.sharespot.iot.core.SensorData&gt; measures = 4;</code>
   */

  /* nullable */
pt.sharespot.iot.core.sensor.buf.SensorData getMeasuresOrDefault(
      int key,
      /* nullable */
pt.sharespot.iot.core.sensor.buf.SensorData defaultValue);
  /**
   * <code>map&lt;int32, .pt.sharespot.iot.core.SensorData&gt; measures = 4;</code>
   */

  pt.sharespot.iot.core.sensor.buf.SensorData getMeasuresOrThrow(
      int key);
}
